...
void Bplus_nominal() {
    ...
    // Common Positive Scale (Fitting Parameter)
    ...
    // ERFC Background (for Partially Reconstructed Background, 
    // present in the Left Sideband)
    ...
    // Fit the Model to Data (Extended Maximum Likelihood Method)
...
// Linear Background Variation Fit for B⁺
...
// 2nd Degree Polynomial Background Variation for B⁺
void Bplus_2degreepoly() {
    const int nbins_plot = 100;
    ...
    RooRealVar c1b("c1b","Bernstein c1", 0.33, 0.0, 10.0);
...
void Bplus_fixedmean() {
    ...
    pave->SetFillColor(0);
    pave->SetBorderSize(1);
    // Signal: Double Gaussian (Fixed Mean)
...
// Triple Gaussian Fit for B⁺ Signal
void Bplus_triplegaussian() {
    ...
    // Components for the Triple Gaussian
    ...
    // Fractions and Propagated Uncertainties
...
// Crystal Ball + Gaussian Signal Fit (with Shared Mean) for B⁺
...
// Fit Without the Left Sideband Variation for B⁺
void Bplus_Without_Left_Sideband() {
    ...
    double xhigh = 5.8;
    double xlow  = min_signal - 0.02; // 0.02 to Expand the Signal Region a Little

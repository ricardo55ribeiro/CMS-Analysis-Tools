...
// Selection and Acceptance Cuts (used for MC Filtering)
#include "ACCSEL.h"
...
// Fit Parameters Initial Values and Limits
// (because variables can't be defined inside ifs)
struct FitParams {
...
// Fit MC Signal for the 'particle' using a Double Gaussian Model
void fit_mc_signal_roofit(TString particle) {
    // Open ROOT File, Get the Tree, and Set Definitions of Histogram (depending on the particle)
    TTree* tree = nullptr;
    ...
    if (!file || file->IsZombie()) {
        std::cerr << "MC File not found!" << std::endl;
        return;
    }
    ...
    // Check if Histogram is not Empty after Cuts 
    if (h->GetEntries() == 0) {
        std::cerr << "Empty Histogram after cuts!" << std::endl;
        delete h;
        return;
    }

    // Normalize Histogram (not in use right now)
    // h->Scale(1.0 / h->Integral());

    // Inicial Values and Limits for each particle (using FitParams)
    FitParams params;
    if (particle == "Bd"){
    ...
    // Name x-axis depending on the particle
    ...
    // Save with specific particle name
    c->SaveAs(file_name);
...
// Fit Bs MC Signal using a Triple Gaussian Model
void fit_mc_signal_Bs() {
    ...
    // Fractions for the Gaussians (Third Fraction is 1 - c1 - c2)
    ...
    // Triple Gaussian as a Normalized Shape:
    // RooAddPdf with 3 PDFs and 2 fractions 
    // => Third Fraction is Implicit (1 - c1 - c2), as Told Above
...
// B0 MonteCarlo Fit with Right-Tag and Wrong-Tag Components
void fit_bd_rt_wt() {
    ...
    // Histogram Binning and Fit Range
    ...
    auto makeData = [&](const char* hname, const TString& cut)->std::pair<TH1F*, RooDataHist*> {
        ...
        if (h->GetEntries() == 0) {
            std::cerr << "Empty histogram after cuts for " << hname << "!" << std::endl;
            delete h;
    ...
    // Helper Lambda to Decorate a Frame, Draw Pulls and Save a Canvas
    auto savePlot = [&](RooPlot* frame,
        ...
        RooHist* pullHist = frame->pullHist(dataObjName, modelObjName);
    ...
    // Simultaneous with RT: Mean is Shared 
    ...
    RooGaussian    gG_WT("gG_WT", "WT Gaussian", Bmass, mean, sigmaG_2); // Mean is Fixed from Fit 1
    ...
    // Separate Yields N_RT and N_WT (to Check the Yield of Each Component)
